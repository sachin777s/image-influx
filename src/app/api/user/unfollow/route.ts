...
export async function PUT(request: NextRequest) {
    try {
        ...
        if (!isFollowed) {
            return NextResponse.json("Oops! You have't followed", { status: 409 })
        }

        await sql`
        UPDATE Users
        SET followings = ARRAY_REMOVE(followings, ${followingId})
        WHERE id = ${userId}
        `;
        ...
        `;
        return NextResponse.json("Unfollow Successfull", { status: 200 });
